...
const getStart = () => {
    if (startPromise)
        return startPromise;
    startPromise = Promise.resolve().then(() => __importStar(require("smoldot"))).then((sm) => {
...
const getStartWithByteCode = () => {
    ...
    startWithByteCodePromise = Promise.resolve().then(() => __importStar(require("smoldot/no-auto-bytecode"))).then((sm) => sm.startWithBytecode);
    return startWithByteCodePromise;
};
...
const getClientAndIncRef = (config) => {
    ...
    const newClientPromise = worker
        ? getStartWithByteCode().then((start) => {
            return start(Object.assign(Object.assign({}, clientOptions), { bytecode: new Promise((resolve) => {
...
const createScClient = (config) => {
    ...
    const addChain = (chainSpec, jsonRpcCallback, potentialRelayChains, databaseContent) => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
    ...
    const addWellKnownChain = (supposedChain, jsonRpcCallback, databaseContent) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            return yield addChain(yield (0, index_js_1.getSpec)(supposedChain), jsonRpcCallback, undefined, databaseContent);
        }
        finally {
            decRef(configOrDefault);
        }
