...
var zombieHandler_exports = {};
__export(zombieHandler_exports, {
  zombieHandler: () => zombieHandler
});
module.exports = __toCommonJS(zombieHandler_exports);
...
function parseConfigSync(filePath) {
  let result;
  const file = (0, import_fs3.readFileSync)(filePath, "utf8");
  switch ((0, import_path2.extname)(filePath)) {
    case ".json":
      result = JSON.parse(file);
      break;
...
function parseRunCmd(launchSpec, additionalRepos) {
  ...
  const args = launchSpec.options ? [...launchSpec.options] : fetchDefaultArgs(import_path3.default.basename(launchSpec.binPath), additionalRepos);
  if (launchSpec.ports) {
...
async function launchNode(cmd, args, name) {
  ...
  const dirPath = import_path4.default.join(process.cwd(), "tmp", "node_logs");
  const onProcessExit = () => {
  ...
  const logLocation = import_path4.default.join(
    dirPath,
    `${import_path4.default.basename(cmd)}_node_${args.find((a) => a.includes("port"))?.split("=")[1]}_${runningNode.pid}.log`
    ...
    context: {
      ...
      upgradeRuntime: async (options = {}) => {
        ...
        const params = {
          ...
          from: options.from || import_util4.alith
        };
        if (options.logger) {
