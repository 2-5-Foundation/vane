...
async function createFolders() {
  await import_promises3.default.mkdir("scripts").catch(() => "scripts folder already exists, skipping");
  await import_promises3.default.mkdir("tests").catch(() => "tests folder already exists, skipping");
  await import_promises3.default.mkdir("tmp").catch(() => "tmp folder already exists, skipping");
}
async function generateConfig() {
  for (; ; ) {
    if (await import_promises3.default.access("moonwall.config.json").catch(() => true)) {
      ...
      await import_promises3.default.writeFile("moonwall.config", textBlob + JSONBlob, "utf-8");
...
function clearNodeLogs(silent = true) {
  ...
  for (const file of files) {
    ...
    if (file.endsWith(".log")) {
      import_fs2.default.unlinkSync(import_path3.default.join(dirPath, file));
    }
  }
}
function reportLogLocation(silent = false) {
  const dirPath = import_path3.default.join(process.cwd(), "tmp", "node_logs");
  const result = import_fs2.default.readdirSync(dirPath);
  let consoleMessage = "";
  let filePath = "";
  try {
    filePath = process.env.MOON_ZOMBIE_DIR ? process.env.MOON_ZOMBIE_DIR : process.env.MOON_LOG_LOCATION ? process.env.MOON_LOG_LOCATION : import_path3.default.join(dirPath, result.find((file) => import_path3.default.extname(file) == ".log"));
...
async function checkExists(path10) {
  ...
  const fsResult = import_node_fs2.default.existsSync(binPath);
  if (!fsResult) {
    throw new Error(
...
async function downloadBinsIfMissing(binPath) {
  ...
  const binPathExists = import_node_fs2.default.existsSync(binPath);
  if (!binPathExists && process.arch === "x64") {
    const choices = await import_inquirer2.default.prompt({
      name: "download",
      type: "list",
      message: `The binary ${import_chalk2.default.bgBlack.greenBright(
        binName
      )} is missing from ${import_chalk2.default.bgBlack.greenBright(
...
Would you like to download it now?`,
    ...
    if (!choices.download) {
      ...
      (0, import_child_process2.execSync)(`mkdir -p ${binDir}`);
      (0, import_child_process2.execSync)(`pnpm moonwall download ${binName} latest ${binDir}`, {
        stdio: "inherit"
      });
    }
  } else if (!binPathExists) {
    console.log(
      `The binary: ${import_chalk2.default.bgBlack.greenBright(
        binName
      )} is missing from: ${import_chalk2.default.bgBlack.greenBright(import_node_path.default.join(process.cwd(), binDir))}`
    );
    console.log(
      `Given you are running ${import_chalk2.default.bgBlack.yellowBright(
        process.arch
...
function checkListeningPorts(processId) {
  try {
    const stdOut = (0, import_child_process2.execSync)(`lsof -p  ${processId} | grep LISTEN`, { encoding: "utf-8" });
    ...
    console.log(e);
    const binName = (0, import_child_process2.execSync)(`ps -p ${processId} -o comm=`).toString().trim();
    console.log(
...
function checkAlreadyRunning(binaryName) {
  try {
    console.log(`Checking if ${import_chalk2.default.bgWhiteBright.blackBright(binaryName)} is already running...`);
    const stdout = (0, import_child_process2.execSync)(`pgrep ${[binaryName.slice(0, 14)]}`, {
      encoding: "utf8",
      timeout: 2e3
    });
...
${pids.map((pid) => {
  ...
  switch (choice.AlreadyRunning) {
    case "kill":
      pids.forEach((pid) => {
        (0, import_child_process2.execSync)(`kill ${pid}`);
      });
      break;
    case "continue":
      break;
...
function checkAccess(path10) {
  const binPath = path10.split(" ")[0];
  try {
    import_node_fs2.default.accessSync(binPath, import_node_fs2.default.constants.X_OK);
  } catch (err) {
...
async function getBinaryArchitecture(filePath) {
  return new Promise((resolve, reject) => {
    ...
    import_node_fs2.default.open(filePath, "r", (err, fd) => {
      if (err) {
        reject(err);
        return;
      }
      const buffer = Buffer.alloc(20);
      import_node_fs2.default.read(fd, buffer, 0, 20, 0, (err2, bytesRead, buffer2) => {
        if (err2) {
          reject(err2);
          return;
        }
...
async function devBinCheck(env) {
  ...
  const binName = import_path4.default.basename(env.foundation.launchSpec[0].binPath);
...
async function executeScript(scriptCommand, args) {
  ...
  try {
    ...
    const ext = import_path4.default.extname(script);
    const scriptPath = import_path4.default.join(process.cwd(), scriptsDir, scriptCommand);
    if (!files.includes(script)) {
      throw new Error(`Script ${script} not found in ${scriptsDir}`);
    }
    console.log(`========== Executing script: ${import_chalk3.default.bgGrey.greenBright(script)} ==========`);
    switch (ext) {
      case ".js":
        (0, import_child_process3.execSync)("node " + scriptPath + ` ${args}`, { stdio: "inherit" });
        break;
      case ".ts":
        (0, import_child_process3.execSync)("pnpm tsx " + scriptPath + ` ${args}`, { stdio: "inherit" });
        break;
      case ".sh":
        (0, import_child_process3.execSync)(scriptPath + ` ${args}`, { stdio: "inherit" });
        break;
      default:
        console.log(`${ext} not supported, skipping ${script}`);
    }
  } catch (err) {
    console.error(`Error executing script: ${import_chalk3.default.bgGrey.redBright(err)}`);
  }
}
...
function parseRunCmd(launchSpec, additionalRepos) {
  ...
  const args = launchSpec.options ? [...launchSpec.options] : fetchDefaultArgs(import_path5.default.basename(launchSpec.binPath), additionalRepos);
  if (launchSpec.ports) {
...
function getZombieConfig(path10) {
  const fsResult = import_node_fs3.default.existsSync(path10);
  if (!fsResult) {
    throw new Error(
      `No ZombieConfig file found at location: ${path10} 
 Are you sure your ${import_chalk5.default.bgWhiteBright.blackBright(
        "moonwall.config.json"
      )} file has the correct "configPath" in zombieSpec?`
    );
  }
  const buffer = import_node_fs3.default.readFileSync(path10, "utf-8");
    ...
    client.on("data", async (data) => {
      ...
      if (response.status === "success") {
        ...
        for (let i = 0; ; i++) {
          ...
          await (0, import_promises4.setTimeout)(200);
        }
        resolve(response);
      }
...
async function launchNode(cmd, args, name) {
  ...
  const dirPath = import_path6.default.join(process.cwd(), "tmp", "node_logs");
  const onProcessExit = () => {
  ...
  const runningNode = (0, import_child_process4.spawn)(cmd, args);
  const logLocation = import_path6.default.join(
    dirPath,
    `${import_path6.default.basename(cmd)}_node_${args.find((a) => a.includes("port"))?.split("=")[1]}_${runningNode.pid}.log`
...
async function findPortsByPid(pid, retryCount = 600, retryDelay = 100) {
  for (let i = 0; i < retryCount; i++) {
    try {
      const stdout = (0, import_child_process4.execSync)(`lsof -i -n -P | grep LISTEN | grep ${pid}`).toString();
      const ports = [];
...
var debug2 = (0, import_debug3.default)("global:providers");
...
var debugSetup = (0, import_debug4.default)("global:context");
var MoonwallContext = class _MoonwallContext {
  ...
  async startZombieNetwork() {
    ...
    const onProcessExit = () => {
      try {
        ...
        (0, import_node_child_process2.exec)(`kill ${processIds.join(" ")}`, (error) => {
          if (error) {
        ...
        try {
          ...
          switch (message.cmd) {
            ...
            case "restart": {
              ...
              await (0, import_promises5.setTimeout)(1e3);
            ...
            case "kill": {
              ...
              const result = (0, import_node_child_process2.exec)(`kill ${pid}`, { timeout: 1e3 });
              writeToClient({
                status: "success",
      ...
      const promises2 = this.providers.filter(({ type }) => type == "polkadotJs").filter(
        ...
        return await new Promise(async (resolve) => {
          ...
          while ((await provider.api.rpc.chain.getBlock()).block.header.number.toNumber() == 0) {
            await (0, import_promises5.setTimeout)(500);
          }
  ...
  static async destroy() {
    ...
    if (ctx.zombieNetwork) {
      ...
      try {
        (0, import_child_process5.execSync)(`kill ${processIds.join(" ")}`, {});
      } catch (e) {
...
async function isPidRunning(pid) {
  const output = (0, import_node_child_process2.exec)(`ps -p ${pid} -o pid=`);
...
async function executeTests(env, additionalArgs) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const folders = env.testFileDir.map((folder) => import_path7.default.join(".", folder, "/"));
...
async function runNetworkCmd(args) {
  ...
  if (!env) {
    const envList = globalConfig.environments.map((env2) => env2.name);
    throw new Error(
      `No environment found in config for: ${import_chalk7.default.bgWhiteBright.blackBright(
        args.envName
      )}
 Environments defined in config are: ${envList}
`
  ...
  const questions2 = [
    ...
    },
    {
      name: "MenuChoice",
      type: "list",
      message: `Environment : ${import_chalk7.default.bgGray.cyanBright(args.envName)}
Please select a choice: `,
      ...
      choices: [
        ...
        {
          name: foundation == "dev" || foundation == "chopsticks" || foundation == "zombie" ? `Command:   Run command on network (${import_chalk7.default.bgGrey.cyanBright(foundation)})` : import_chalk7.default.dim(
            `Not applicable for foundation type (${import_chalk7.default.bgGrey.cyanBright(foundation)})`
          ),
          value: 3,
          short: "cmd",
        ...
        {
          name: testFileDirs.length > 0 ? "Test:      Execute tests registered for this environment   (" + import_chalk7.default.bgGrey.cyanBright(testFileDirs) + ")" : import_chalk7.default.dim("Test:    NO TESTS SPECIFIED"),
          value: 4,
          disabled: testFileDirs.length > 0 ? false : true,
          short: "test"
        },
        {
          name: testFileDirs.length > 0 ? "GrepTest:  Execute individual test(s) based on grepping the name / ID (" + import_chalk7.default.bgGrey.cyanBright(testFileDirs) + ")" : import_chalk7.default.dim("Test:    NO TESTS SPECIFIED"),
          value: 5,
          disabled: testFileDirs.length > 0 ? false : true,
          short: "grep"
        },
  ...
  if (!args.GrepTest) {
    await import_inquirer3.default.prompt(questions2.find(({ name }) => name == "NetworkStarted"));
  } else {
    process.env.MOON_RECYCLE = "true";
  ...
  mainloop:
    for (; ; ) {
      const choice = await import_inquirer3.default.prompt(questions2.find(({ name }) => name == "MenuChoice"));
      ...
      switch (choice.MenuChoice) {
        ...
        case 5:
          ...
          break;
        case 6: {
          const quit = await import_inquirer3.default.prompt(questions2.find(({ name }) => name == "Quit"));
          if (quit.Quit === true) {
            break mainloop;
          }
          break;
...
var resolveZombieCommandChoice = async () => {
  ...
  if (choice.cmd == "back") {
    ...
    const whichNode = await import_inquirer3.default.prompt({
      name: "nodeName",
      type: "input",
...
var resolveCommandChoice = async () => {
  ...
  switch (choice.cmd) {
    ...
    case "createUnfinalizedBlock":
      ...
      break;
    case "createNBlocks": {
      const result = await new import_inquirer3.default.prompt({
        name: "n",
        type: "number",
...
var resolveInfoChoice = async (env) => {
  console.log(import_chalk7.default.bgWhite.blackBright("Node Launch args:"));
  console.dir(MoonwallContext.getContext().environment, { depth: null });
  console.log(import_chalk7.default.bgWhite.blackBright("Launch Spec in Config File:"));
...
var resolveTailChoice = async (env) => {
  ...
  const resumePauseProse = [
    `, ${import_chalk7.default.bgWhite.black("[p]")} Pause tail`,
    `, ${import_chalk7.default.bgWhite.black("[r]")} Resume tail`
  ];
  const bottomBarBase = `\u{1F4DC} Tailing Logs, commands: ${import_chalk7.default.bgWhite.black(
    "[q]"
  )} Quit, ${import_chalk7.default.bgWhite.black("[t]")} Test, ${import_chalk7.default.bgWhite.black("[g]")} Grep test`;
  bottomBarContents = bottomBarBase + resumePauseProse[0];
  const ui = new import_inquirer3.default.ui.BottomBar({
  ...
  for (; ; ) {
    ...
    if (process.env.MOON_ZOMBIE_NODES) {
      ...
      zombieContent = `, ${import_chalk7.default.bgWhite.black("[,]")} Next Log, ${import_chalk7.default.bgWhite.black(
        "[.]"
      )} Previous Log  | CurrentLog: ${import_chalk7.default.bgWhite.black(
