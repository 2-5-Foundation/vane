...
__export(src_exports, {
  ...
  afterAll: () => import_vitest5.afterAll,
  afterEach: () => import_vitest5.afterEach,
  beforeAll: () => import_vitest5.beforeAll,
  beforeEach: () => import_vitest5.beforeEach,
...
var import_api_augment10 = require("@moonbeam-network/api-augment");
__reExport(src_exports, require("@moonwall/types"), module.exports);
...
function fetchCompiledContract(contractName) {
  ...
  const json = (0, import_fs3.readFileSync)(compiledPath, "utf8");
...
async function interactWithPrecompileContract(context, callOptions) {
  ...
  const precompileInfo = import_util2.PRECOMPILES[precompileName];
  if (!precompileInfo) {
...
async function interactWithContract(context, callOptions) {
  const {
    ...
    gas = "estimate",
    value = 0n,
    privateKey = import_util2.ALITH_PRIVATE_KEY,
    rawTxOnly = false,
    call = false
  } = callOptions;
  ...
  if (!call && rawTxOnly) {
    return web3Library === "viem" ? (0, import_util2.createViemTransaction)(context, {
      to: contractAddress,
      data,
      gas: gasParam,
      privateKey,
      value
    }) : (0, import_util2.createEthersTransaction)(context, {
      to: contractAddress,
      data,
      gas: gasParam,
...
async function deployCreateCompiledContract(context, contractName, options) {
  ...
  const { privateKey = import_util2.ALITH_PRIVATE_KEY, args = [], ...rest } = options || {};
  const blob = {
    ...rest,
    privateKey,
    args
  };
  const { contractAddress, logs, status, hash } = await (0, import_util2.deployViemContract)(
    context,
    abi,
    bytecode,
    blob
...
async function checkExists(path7) {
 ...
 Are you sure your ${import_chalk3.default.bgWhiteBright.blackBright(
  ...
  if (binArch !== currentArch && binArch !== "unknown") {
    throw new Error(
      `The binary architecture ${import_chalk3.default.bgWhiteBright.blackBright(
        binArch
      )} does not match this system's architecture ${import_chalk3.default.bgWhiteBright.blackBright(
        currentArch
      )}
Download or compile a new binary executable for ${import_chalk3.default.bgWhiteBright.blackBright(
        currentArch
      )} `
    );
  }
...
async function launchNode(cmd, args, name) {
  ...
  const runningNode = (0, import_child_process2.spawn)(cmd, args);
...
async function findPortsByPid(pid, retryCount = 600, retryDelay = 100) {
  for (let i = 0; i < retryCount; i++) {
    try {
      const stdout = (0, import_child_process2.execSync)(`lsof -i -n -P | grep LISTEN | grep ${pid}`).toString();
      const ports = [];
...
var ProviderFactory = class _ProviderFactory {
  constructor(providerConfig) {
    ...
    this.privateKey = process.env.MOON_PRIV_KEY || import_util3.ALITH_PRIVATE_KEY;
  }
  url;
  privateKey;
  create() {
  ...
  createEthers() {
    ...
    return {
      ...
      type: this.providerConfig.type,
      connect: () => {
        const provider = new import_ethers2.ethers.WebSocketProvider(this.url);
        return new import_ethers2.Wallet(this.privateKey, provider);
      }
    };
  }
  createViem() {
    ...
    return {
      name: this.providerConfig.name,
      type: this.providerConfig.type,
      connect: async () => (0, import_viem2.createWalletClient)({
        chain: await (0, import_util3.deriveViemChain)(this.url),
        account: (0, import_accounts2.privateKeyToAccount)(this.privateKey),
        transport: (0, import_viem2.http)(this.url.replace("ws", "http"))
...
var MoonwallContext = class _MoonwallContext {
  ...
  static async destroy() {
    ...
    if (ctx.zombieNetwork) {
      ...
      try {
        (0, import_child_process3.execSync)(`kill ${processIds.join(" ")}`, {});
      } catch (e) {
...
var COUNCIL_MEMBERS = [import_util4.baltathar, import_util4.charleth, import_util4.dorothy];
var COUNCIL_THRESHOLD = Math.ceil(COUNCIL_MEMBERS.length * 2 / 3);
var TECHNICAL_COMMITTEE_MEMBERS = [import_util4.alith, import_util4.baltathar];
var TECHNICAL_COMMITTEE_THRESHOLD = Math.ceil(
  TECHNICAL_COMMITTEE_MEMBERS.length * 2 / 3
);
var notePreimage = async (context, proposal, account = import_util4.alith) => {
...
var execCouncilProposal = async (context, polkadotCall, index = -1, voters = COUNCIL_MEMBERS, threshold = COUNCIL_THRESHOLD) => {
  ...
  const { result: proposalResult } = await context.createBlock(
    context.polkadotJs().tx.councilCollective.propose(threshold, polkadotCall, lengthBound).signAsync(import_util4.charleth)
  );
  if (threshold <= 1) {
  ...
  return await context.createBlock(
    context.polkadotJs().tx.councilCollective.close(
      ...
      lengthBound
    ).signAsync(import_util4.dorothy)
  );
};
...
var maximizeConvictionVotingOf = async (context, voters, refIndex) => {
  const fee = (await context.polkadotJs().tx.convictionVoting.vote(refIndex, {
    Standard: {
      ...
      balance: (await context.polkadotJs().query.system.account(import_util4.alith.address)).data.free
    }
  }).paymentInfo(import_util4.alith)).partialFee;
  await context.createBlock(
    voters.map(
...
var execTechnicalCommitteeProposal = async (context, polkadotCall, voters = TECHNICAL_COMMITTEE_MEMBERS, threshold = TECHNICAL_COMMITTEE_THRESHOLD) => {
  ...
  const { result: closeResult } = await context.createBlock(
    context.polkadotJs().tx.techCommitteeCollective.close(
      ...
      lengthBound
    ).signAsync(import_util4.baltathar)
  );
  return closeResult;
};
var executeProposalWithCouncil = async (api, encodedHash) => {
  let nonce = (await api.rpc.system.accountNextIndex(import_util4.alith.address)).toNumber();
  ...
  await Promise.all([
    api.tx.councilCollective.propose(1, external, external.length).signAndSend(import_util4.alith, { nonce: nonce++ }),
    api.tx.techCommitteeCollective.propose(1, fastTrack, fastTrack.length).signAndSend(import_util4.alith, { nonce: nonce++ }),
    api.tx.democracy.vote(referendumNextIndex, {
      Standard: {
        balance: voteAmount,
        vote: { aye: true, conviction: 1 }
      }
    }).signAndSend(import_util4.alith, { nonce: nonce++ })
  ]);
...
var cancelReferendaWithCouncil = async (api, refIndex) => {
  ...
  let nonce = (await api.rpc.system.accountNextIndex(import_util4.alith.address)).toNumber();
  await api.tx.democracy.notePreimage(encodedProposal).signAndSend(import_util4.alith, { nonce: nonce++ });
...
async function getWsFromConfig(providerName) {
  if (providerName) {
    ...
    if (typeof provider == "undefined") {
      throw new Error(`Cannot find provider ${import_chalk5.default.bgWhiteBright.blackBright(providerName)}`);
    }
    if (!provider.ws) {
    ...
    if (typeof provider == "undefined") {
      throw new Error(
        `Cannot find providers of type ${import_chalk5.default.bgWhiteBright.blackBright("polkadotJs")}`
      );
    }
    if (!provider.ws) {
      throw new Error("Provider does not have an attached ws() property ");
    ...
    const match = options.expectEvents.every((eEvt) => {
      ...
      if (!found) {
        options.logger ? options.logger(
          `Event ${import_chalk5.default.bgWhiteBright.blackBright(eEvt.meta.name)} not present in block`
        ) : console.error(
          `Event ${import_chalk5.default.bgWhiteBright.blackBright(eEvt.meta.name)} not present in block`
        );
      }
...
async function upgradeRuntimeChopsticks(context, path7) {
  if (!(0, import_fs6.existsSync)(path7)) {
    throw new Error("Runtime wasm not found at path: " + path7);
  }
  const rtWasm = (0, import_fs6.readFileSync)(path7);
...
async function upgradeRuntime(api, preferences) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const code = import_fs6.default.readFileSync(
...
`);
          ...
          log(
            `Registering preimage (${(0, import_ethers3.sha256)(Buffer.from(code))} [~${Math.floor(
              code.length / 1024
            )} kb])...`
          );
        ...
        log(
          `Sending sudo.setCode (${(0, import_ethers3.sha256)(Buffer.from(code))} [~${Math.floor(
            code.length / 1024
          )} kb])...`
        );
      ...
      log(`Waiting to apply new runtime (${import_chalk6.default.red(`~4min`)})...`);
...
var chopsticksHandler = ({
  ...
  const newKeyring = () => {
    ...
    return {
      alice: keyring.addFromUri(isEth ? import_util5.ALITH_PRIVATE_KEY : "//Alice", { name: "Alice default" }),
      bob: keyring.addFromUri(isEth ? import_util5.BALTATHAR_PRIVATE_KEY : "//Bob", { name: "Bob default" }),
      charlie: keyring.addFromUri(isEth ? import_util5.CHARLETH_PRIVATE_KEY : "//Charlie", {
        name: "Charlie default"
      }),
      dave: keyring.addFromUri(isEth ? import_util5.DOROTHY_PRIVATE_KEY : "//Dave", { name: "Dave default" })
    };
  };
  const ctx = {
    ...context,
...
function returnSigner(options) {
  return "privateKey" in options.signer && "type" in options.signer ? (0, import_util6.generateKeyringPair)(options.signer.type, options.signer.privateKey) : options.signer;
}
function returnDefaultSigner() {
  return isEthereumDevConfig() ? import_util6.alith : new import_api3.Keyring({ type: "sr25519" }).addFromUri("//Alice", { name: "Alice default" });
}
async function createDevBlock(context, options, transactions) {
    ...
    if (typeof call == "string") {
      results.push({
        ...
        hash: containsViem ? (await context.viem().request({
          ...
          params: [call]
        })).result : (await (0, import_util6.customWeb3Request)(context.web3(), "eth_sendRawTransaction", [call])).result
      });
  ...
  const blockResult = await (0, import_util6.createAndFinalizeBlock)(api, parentHash, finalize);
  if (results.length == 0) {
    return {
      block: blockResult
    };
  ...
  if (options.expectEvents && options.expectEvents.length > 0) {
    const match = options.expectEvents.every((eEvt) => {
      ...
      if (!found) {
        options.logger ? options.logger(
          `Event ${import_chalk7.default.bgWhiteBright.blackBright(eEvt.meta.name)} not present in block`
        ) : console.error(
          `Event ${import_chalk7.default.bgWhiteBright.blackBright(eEvt.meta.name)} not present in block`
        );
      }
