...
var chopsticksHandler_exports = {};
__export(chopsticksHandler_exports, {
  chopsticksHandler: () => chopsticksHandler
});
module.exports = __toCommonJS(chopsticksHandler_exports);
...
function getZombieConfig(path5) {
  const fsResult = import_node_fs2.default.existsSync(path5);
  if (!fsResult) {
    throw new Error(
      `No ZombieConfig file found at location: ${path5} 
 Are you sure your ${import_chalk3.default.bgWhiteBright.blackBright(
        "moonwall.config.json"
      )} file has the correct "configPath" in zombieSpec?`
    );
  }
  const buffer = import_node_fs2.default.readFileSync(path5, "utf-8");
...
// src/lib/upgradeProcedures.ts
async function upgradeRuntimeChopsticks(context, path5) {
  if (!(0, import_fs4.existsSync)(path5)) {
    throw new Error("Runtime wasm not found at path: " + path5);
  }
  const rtWasm = (0, import_fs4.readFileSync)(path5);
